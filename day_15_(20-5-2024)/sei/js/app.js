...
var listCard = [
    ...
    },
    {
        ...
        content:"Sei V2 introduces a unique architecture designed to enhance scalability and developer flexibility. enhance scalability and developer."
    },
    {
...
var listFaq = document.querySelectorAll('.faq__item')
var imgSlide = document.querySelector('.list-img img')
var curentIndex = 0;
listFaq.forEach((item, index)=>{
    item.addEventListener('click',function(){
        curentIndex = index
        
         var faqInner = this.querySelector('.faq__inner');
        if (faqInner.classList.contains('expanded')) {
                faqInner.classList.remove('expanded');

            } else {
                faqInner.classList.add('expanded')
            }
            showImg(curentIndex)
    })
}
...
function showImg(){
    imgSlide.src = '/img/img'+(curentIndex+1) +'.webp'
}